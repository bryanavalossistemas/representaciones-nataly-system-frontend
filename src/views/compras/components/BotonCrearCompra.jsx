...
export default function BotonCrearCompra({ productos, proveedores }) {
  ...
  const resetCompra = useComprasStore((state) => state.resetCompra);
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: compraService.create,
    onError: (error) => {
      setOpen(false);
  ...
  const handleCreateCompra = () => {
    ...
    if (!error) mutate({ payload: { proveedorId, detallesCompra } });
  };
  ...
  return (
    <>
      ...
      >
        ...
        <span className="font-semibold">Crear Compra</span>
      </Button>
